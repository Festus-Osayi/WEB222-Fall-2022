...
function observationSummary(data) {
  ...
  for (var i = 0; i < data.results.length; i++) {
    console.log(
      `${data.results[i].id}@${data.results[i].observed_on_details.date}: ${data.results[i].species_guess}`
    );
  }
}
...
function observationSummary2(data) {
  ...
  data.results.forEach((elem) =>
    console.log(`${elem.id}@${elem.observed_on_details.date}: ${elem.species_guess}`)
  );
}
...
function observationsByPrivacy(data, geoPrivacy) {
  if (
    !(
      (geoPrivacy === geoPrivacy.toLowerCase()) === 'open' ||
      geoPrivacy.toLowerCase() === 'hidden' ||
      geoPrivacy === null
    )
  ) {
    throw new Error('');
    ...
    for (const element of data.results) {
      if (
        (geoPrivacy === geoPrivacy.toLowerCase()) === 'open' ||
        geoPrivacy === null ||
        geoPrivacy.toUpperCase() === 'HIDDEN'
      ) {
        if (element.geoPrivacy === geoPrivacy) {
          arrSize.push(element);
        }
      } else {
        if (element.geoPrivacy === geoPrivacy.toLowerCase() || geoPrivacy.toUpperCase()) {
          arrSize.push(element);
        }
      }
    }
...
function transformObservation(original) {
  // TODO
  var coordsArray = original.location.split(',');
  var photoURLArray = [];

  for (let i = 0; i < original.photos.length; i++) {
    photoURLArray[i] = original.photos[i].url;
  }
...
function transformObservations(data) {
  ...
  var arrayTransform = [];
  data.results.forEach((element) => arrayTransform.push(transformObservation(element)));
...
function transformObservations2(data) {
  // TODO - use map()
  let arrayTransform = data.results.map((element) => transformObservation(element));
...
function getObservationsById(data, ...ids) {
  // TODO
  while (ids.length === 1) {
    var getResults = data.results.find((element) => element.id === ids[0]);
    while (getResults) {
      return getResults;
    }
    return null;
  ...
  var foundElement;
  var getObservationId = [];

  ids.forEach(function (id) {
    foundElement = data.results.find((element) => element.id === id);
    if (foundElement) {
      getObservationId.push(foundElement);
    }
  });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  // TODO
  var observationsAccuracy = [];

  if (options.greaterThan && options.lessThan && options.equal) {
    observationsAccuracy = data.results.filter(
      (element) => element.positional_accuracy === options.equal
    );
  } else if (options.greaterThan && options.lessThan) {
    observationsAccuracy = data.results.filter(
      (element) =>
        element.positional_accuracy > options.greaterThan &&
        element.positional_accuracy < options.lessThan
    );
  } else if (options.greaterThan) {
    observationsAccuracy = data.results.filter(
      (element) => element.positional_accuracy > options.greaterThan
    );
  } else if (options.lessThan) {
    observationsAccuracy = data.results.filter(
      (element) => element.positional_accuracy < options.lessThan
    );
    ...
    observationsAccuracy = data.results;
  }

  return observationsAccuracy;
}
...
function getTaxonPhotos(data) {
  ...
  var arrObject = [];
  for (var i = 0; i < data.results.length; i++) {
    while (!data.results[i].taxon) {
      return [];
    }
    if (!(data.results[i].taxon.default_photo === false)) {
      urlObjects = 'https://static.inaturalist.org/photos/575727/';

      photoObject = {
        original: urlObjects + 'original.jpg?' + data.results[i].id,
        square: urlObjects + 'square.jpg?' + data.results[i].id,
        small: urlObjects + 'small.jpg?' + data.results[i].id,
        medium: urlObjects + 'medium.jpg?' + data.results[i].id,
        large: urlObjects + 'large.jpg?' + data.results[i].id
      };

      arrObject.push(photoObject);
    }
  }
...
function getUserStats(data) {
  // TODO
  var user_count = data.results.length;
  var getObservations = 0;
  var getJournals = 0;
  var getSpecies = 0;
  var x = 0;
  while (x < data.results.length) {
    getObservations += data.results[x].user.observations_count;
    getJournals += data.results[x].user.journal_posts_count;
    getSpecies += data.results[x].user.species_count;
    x++;
  }
  ...
  let returnObj = {
    ...
    averages: {
      observations: getObservations / user_count,
      journals: getJournals / user_count,
      species: getSpecies / user_count
    }
  };
...
function extractUserLogins(data) {
  ...
  while (i < data.results.length) {
    if (!timeZone.includes(data.results[i].user.login)) {
      timeZone.push(data.results[i].user.login);
    }
    i++;
  }
...
function extractUserLogins2(data) {
  ...
  while (i < data.results.length) {
    set.add(data.results[i].user.login);
    i++;
  }
